...
def extract_book_metadata(md_text: str) -> Dict[str, Any]:
    ...
    """
    metadata: Dict[str, Optional[str]] = {"title": None, "author": None, "series": None}
...
def process_document(
    file_path=None, chunk_size=512, chunk_overlap=50, batch_size=BATCH_SIZE
):
    """
    ...
    char_per_second = len("".join([item.page_content for item in documents])) / (
        end - start
    )
    print(
        f"Time taken: {end - start:.2f} seconds / {char_per_second:,.2f} characters per second"
    )
...
def store_in_qdrant(
    documents_embeddings, book_metadata=None, collection_name="recursive"
):
    ...
    for i, (doc, embeddings) in enumerate(documents_embeddings):
        ...
        points.append(
            PointStruct(
                id=str(uuid.uuid4()),
                vector=embeddings,
                payload={"text": doc.page_content, "metadata": chunk_metadata},
            )
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process a document and store embeddings"
    )
    parser.add_argument(
        "--file_path",
        type=str,
    ...
    parser.add_argument(
        "--chunk-size", type=int, default=512, help="Maximum size of each chunk"
    )
    parser.add_argument(
        "--chunk-overlap", type=int, default=50, help="Overlap between chunks"
    )
    parser.add_argument(
        "--collection-name", default="recursive", help="Name of the Qdrant collection"
    )
