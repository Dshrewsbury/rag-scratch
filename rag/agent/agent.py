...
class Agent:
    def __init__(self, model_config=None):
        ...
        """
        self.system_prompt = system_prompt_txt.read_text() if system_prompt_txt.exists() else ""
    ...
    async def generate_response_stream(self, conversation_id: str, query: str) -> AsyncIterator[str]:
        """
            ...
            async for chunk in response:
                ...
                if chunk.choices and hasattr(chunk.choices[0], 'delta') and hasattr(chunk.choices[0].delta, 'tool_calls'):
                    ...
                    for tool_call in tool_calls:
                        if not tool_call.get("id") or tool_call["id"] in tool_calls_seen:
                            continue
                        ...
                        async for add_chunk in additional_response:
                            if add_chunk.choices and hasattr(add_chunk.choices[0], 'delta') and add_chunk.choices[0].delta.content:
                                token = add_chunk.choices[0].delta.content
                                assistant_message["content"] += token
                                yield token
    ...
    async def generate_response(self, conversation_id: str, query: str) -> str:
        ...
        try:
            ...
            logger.info(f"Generating complete response with model: {self.model_config.get('model')}")
    ...
    async def _search(self, query: str, limit: int = 3) -> str:
        ...
        try:
            ...
            query_embedding = await loop.run_in_executor(
                None, 
                lambda: self.embedding_generator.generate_embeddings(query)
            )
